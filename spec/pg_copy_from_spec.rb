...
describe "COPY FROM" do
  ...
  it "should import from file if path is passed without field_map" do
    ...
    TestModel.order(:id).load.map{|r| r.attributes}.should == [{'id' => 1, 'data' => 'test data 1'}]
  end
  ...
  it "should import and allow changes in block" do
    ...
    TestModel.order(:id).load.map{|r| r.attributes}.should == [{'id' => 1, 'data' => 'changed this data'}]
  end
  ...
  it "default set of columns should be all table columns minus [id, created_at, updated_at]" do
    ExtraField.pg_upsert(File.open(File.expand_path('spec/fixtures/comma_with_header.csv'), 'r'))
    ExtraField.order(:id).load.map{|r| r.attributes}.should == [{'id' => 1, 'data' => 'test data 1', 'created_at' => nil, 'updated_at' => nil}]
  end
  ...
  it "should use the table name given by :table" do
    ActiveRecord::Base.pg_upsert(File.open(File.expand_path('spec/fixtures/comma_without_header.csv'), 'r'), :header => false, :columns => [:id,:data], :table => "test_models")
    TestModel.order(:id).load.map{|r| r.attributes}.should == [{'id' => 1, 'data' => 'test data 1'}]
  end
  ...
  it "should copy from even when table fields need identifier quoting" do
    ...
    ReservedWordModel.order(:id).load.map{|r| r.attributes}.should == [{"group"=>"group name", "id"=>1, "select"=>"test select"}]
  end
end
