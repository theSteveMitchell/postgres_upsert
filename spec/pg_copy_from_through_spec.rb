...
describe "COPY FROM with :through_table option" do
  ...
  it "should not violate primary key constraint" do
    expect{
      TestModel.pg_upsert File.expand_path('spec/fixtures/comma_with_header.csv'), :through_table => "test_models_temp"
    }.to_not raise_error
  end

  it "should upsert (update existing records and insert new records)" do
    TestModel.pg_upsert File.expand_path('spec/fixtures/tab_with_two_lines.csv'), :delimiter => "\t", :through_table => "test_models_temp"
    TestModel.order(:id).load.map{|r| r.attributes}.
      should == [{"id"=>1, "data"=>"test data 1"}, {"id"=>2, "data"=>"test data 2"}]
  end

  it "should require columns option if no header" do
    expect{
      TestModel.pg_upsert File.expand_path('spec/fixtures/2_col_binary_data.dat'), :format => :binary, :through_table => "test_models_temp"
